...
userSchema.statics.saveUser = async usrObj => {
    ...
    let count = await UserModel.countDocuments({email: usrObj.email})
    if (count !== 0)
...
userSchema.statics.forgotPasswordRequest = async (email) => {
    console.log("inside forgot password modal code")
...
userSchema.statics.editUser = async userObj => {
    ...
    if (storedUser.email != userObj.email) {
        let count = await UserModel.countDocuments({
            _id: {'$ne': mongoose.Schema.ObjectId(userObj._id)},
            'email': userObj.email
        })
        if (count != 0) {
...
userSchema.statics.updateAddedRole = async (roleInput) => {
    if (!roleInput)
        throw new AppError("Identifier required for Update", IDENTIFIER_MISSING, ErrorCodes.HTTP_BAD_REQUEST)
    let userRoleUpdate
...
userSchema.statics.deleteUser = async (userID) => {
    if (!userID)
        throw new AppError("Identifier required for delete", IDENTIFIER_MISSING, ErrorCodes.HTTP_BAD_REQUEST)
